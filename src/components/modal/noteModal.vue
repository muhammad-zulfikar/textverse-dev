...
<template>
  ...
  <transition name="zoom">
    <div v-if="props.isOpen" class="fixed inset-0 z-50 flex items-center justify-center font-serif">
      ...
      <div @click.stop :class="[
        ...
        <textarea v-if="!uiStore.showPreview" v-model="editedNote.content" placeholder="Content"
          ...
          :style="contentStyle"></textarea>
        <div v-else
          class="pt-4 prose dark:prose-dark markdown-body prism-highlight w-full bg-transparent resize-none overflow-auto flex-grow"
          :style="contentStyle" v-html="notesStore.toggleMarkdownPreview(editedNote)"></div>
      </div>
    </div>
  </transition>
  <AlertModal :is-open="uiStore.isAlertOpen" :message="uiStore.alertMessage" @confirm="confirmDelete"
    @cancel="closeAlert" />
</template>
